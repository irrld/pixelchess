...
void GameCanvas::setup() {
  ...
  for (int i = 1; i <= kBoardThemes; ++i) {
    board_textures[i - 1].loadImage("boards/board_persp_0" + gToStr(i) + ".png");
    board_textures[i - 1].setFiltering(2, 2);
  }
...
void GameCanvas::HighlightPosition(int board_x, int board_y, const gColor& color) {
  ...
  int render_offset_y = (6 + (7 - board_y) * 12) * RenderUtil::kPixelScale;
...
void GameCanvas::Move(int x, int y, int to_x, int to_y) {
  ...
  animate_from_pos_y = (6 + (7 - y) * 12 - piece_offset) * RenderUtil::kPixelScale;
  animate_to_pos_x = (7 + to_x * 16) * RenderUtil::kPixelScale;
  animate_to_pos_y = (6 + (7 - to_y) * 12 - piece_offset) * RenderUtil::kPixelScale;
  animate = true;
...
void GameCanvas::DrawBoard() {
  ...
  board_textures[board_theme].draw(board_pos_x, board_pos_y, board_width, board_height);
  for (int x = 0; x < 8; x++) {
    for (int y = 7; y >= 0; y--) {
      Ref<Piece> piece = chess_board->GetPiece(x, y);
      ...
      int index = piece->GetType();
      int render_offset_x = (7 + x * 16) * RenderUtil::kPixelScale;
      int render_offset_y = (6 + (7 - y) * 12 - piece_offset) * RenderUtil::kPixelScale;
